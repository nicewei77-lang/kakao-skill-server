...
app.post('/kakao', async (req, res) => {
  ...
  if (callbackUrl) {
    ...
    return res.json({
      ...
      useCallback: true,
      data: {
        text: '본인인증을 처리하고 있어요... 잠시만 기다려주세요! 😊',
      },
    });
  }
...
app.post('/attendance', async (req, res) => {
  ...
  console.log('출석 조회 요청 - 사용자 ID:', kakaoUserId);
  console.log('콜백 모드:', callbackUrl ? '활성화' : '비활성화');
  ...
  if (callbackUrl) {
    ...
    (async () => {
      try {
        ...
        console.error('출석 조회 콜백 처리 중 오류:', err);
    ...
    return res.json({
      ...
      useCallback: true,
      data: {
        text: '출석 정보를 조회하고 있어요... 잠시만 기다려주세요! 📊',
      },
    });
  }
